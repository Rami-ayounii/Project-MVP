...
from flask import Flask, render_template, request, flash, redirect, send_file, url_for,session,get_flashed_messages, jsonify
import json
...
app.config['MYSQL_DB']='tutect-database'
...
def uplode():
    if request.method == 'POST':
        ...
        if file and allowed_file(file.filename): 
            ...
            target_filename = filename.rsplit('.', 1)[0] + '_output'
    ...
    messages = get_flashed_messages(with_categories=True)
    return render_template('uplode.html', messages=messages,username=session['username'])


@app.route('/report')
def report():
    # Appel à la fonction all() avec les deux chemins de fichie
    source_path = session.get('source_path')
    target_path = session.get('target_path')
    response_data = allstat(source_path, target_path)
    return render_template('report.html',response_data=response_data,username=session['username'])
